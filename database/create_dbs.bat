...
rem Windows helper for Cloud SQL setup (customer & product DBs only)
...
if "%CREATE_INSTANCE%"=="" set "CREATE_INSTANCE=1"
...
if "%CREATE_INSTANCE%"=="1" (
  ...
  echo Skipping instance creation (CREATE_INSTANCE=0) using existing "%INSTANCE%".
)

echo Creating databases...
for %%D in (customer-database product-database) do (
