...
OUTPUT_DIR="${OUTPUT_DIR:-$REPO_ROOT/runtime/scenario1_cases}"
...
RESPONSE_RUNS="${RESPONSE_RUNS:-10}"
THROUGHPUT_RUNS="${THROUGHPUT_RUNS:-10}"
OPS_PER_CLIENT="${OPS_PER_CLIENT:-1000}"
READ_WEIGHT="${READ_WEIGHT:-3}"
WRITE_WEIGHT="${WRITE_WEIGHT:-2}"
...
kill_frontend_replica() {
  sudo systemctl stop marketplace-buyer-frontend-8001
  ssh marketplace@"$SELLER_HOST" "sudo systemctl stop marketplace-seller-frontend-8101"
}

kill_product_follower() {
  for host in "${PRODUCT_HOSTS[@]}"; do
...
run_case() {
  ...
  local output_file="$OUTPUT_DIR/scenario1_${failure_mode}.json"
  ...
  "$PYTHON_BIN" "$BENCHMARK_SCRIPT" \
    ...
    --throughput-functions "$READ_FN" "$WRITE_FN" \
...
for mode in no_failures frontend_failure product_follower_failure product_leader_failure; do
  echo "Running scenario 1 case: $mode"
  run_case "$mode"
done

echo "Scenario 1 case outputs written to $OUTPUT_DIR"
