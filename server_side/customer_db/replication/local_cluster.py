...
RUNTIME_SQLITE_ROOT = REPO_ROOT / "runtime" / "sqlite"
...
class LocalCustomerDbReplicaCluster:
    ...
    def _create_storage(self) -> None:
        ...
        if self.database_prefix:
            ...
            return
        self._storage_root = RUNTIME_SQLITE_ROOT
        self._storage_root.mkdir(parents=True, exist_ok=True)
        self._ephemeral_storage = False
        for replica_id in range(self.replica_count):
            replica_dir = self._storage_root / f"customer-db-replica_{replica_id}"
            replica_dir.mkdir(parents=True, exist_ok=True)
            db_path = replica_dir / self.sqlite_db_name
            for suffix in ("", "-shm", "-wal"):
                candidate = Path(str(db_path) + suffix)
                if candidate.exists():
                    candidate.unlink()
            self.database_paths.append(str(db_path))

    def _drop_storage(self) -> None:
        self.database_paths.clear()
        if self._ephemeral_storage and self._storage_root is not None:
            import shutil
